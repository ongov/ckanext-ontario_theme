...
def get_some_packages():
  ...
  count = 1
  while start < count:
    get_req = requests.get(package_search_endpoint + '?rows=1000&start={}'.format(start),
                      headers=headers)
    if count == 1:
      ...
      if package['name'] in white_list:
        ...
        if not req.status_code == 200:
            ...
            for r in resources:
              ...
              #urllib2.urlretrieve(r['url'], write_path+"/"+resource_file_name)
              with open(write_path+"/"+resource_file_name,'wb') as f:
                f.write(urllib3.urlopen(r['url']).read())
                f.close()
                del r['url']
              payload = dict(r)
              file_payload = { "package_id": package_id}
              ...
              if resp.status_code == 200:
                  ...
                  if resource_file_name.find(".csv") != -1:
                      ...
                      if resp.json()['success'] == True:
                        time.sleep(10)
                        file_count = get_file_count(write_path+"/"+resource_file_name)
                        datastore_loaded = False
                        tries = 0
                        last_count = 0
                        while datastore_loaded is False and tries < 3:
                          time.sleep(30) # should be about 4500
                          response = requests.get(resource_query_endpoint+"?resource_id="+new_resource_id+"&limit=1")
                          print(response.json())
                          ...
                          if tries == 3:
                            logging.info("%s, %s csv rows are off, moving on.", package['name'], resource_file_name)
                      else:
                        logging.info("%s, %s csv didn't upload properly... fix", package['name'], resource_file_name)                  

                  payload = dict(payload)
                  payload['description_translated'] = json.dumps(payload['description_translated'])
                  metadata_resp = requests.post(resource_patch_endpoint+"?id="+new_resource_id,
                             data=payload,
                             headers=localhost_headers)
                  logging.info("%s", resource_file_name)
                  logging.info("%s %s", metadata_resp.status_code, metadata_resp.reason)
              else:
