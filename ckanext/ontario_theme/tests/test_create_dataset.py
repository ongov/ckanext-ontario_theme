...
class TestCreateDataset(object):
    ...
    def test_package_create_with_invalid_update_frequency(self):
        ...
        As long as schema is correct scheming's validations should handle the
        testing.

        This is more of a reminder to setup the schema properly.

        NOTE: Presets are great unless you start modifying things. (i.e.
        presets add predefined values such as validators, until you add
        your own. Those override the preset. So you 'll have to add in
    ...
    def test_package_create_with_invalid_current_as_of(self):
        '''If dataset is given invalid values should raise Invalid.
        Only testing one per input type that was modified. All date inputs
        follow same pattern in schema. As long as schema is correct
        scheming's validations should handle the testing.
